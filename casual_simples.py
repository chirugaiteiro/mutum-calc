...
# Usamos uma lista que mapeia Nome Comum E Nome Científico para garantir a identificação.
# Fator: 10x (Grupo I) ou 5x (Grupo II)
PROTECTED_SPECIES_MS = {
    # GRUPO I: 10 MUDAS
    "Peroba Rosa": 10, "Aspidosperma polyneuron": 10,
    "Cedro": 10, "Cedrela fissilis": 10,
    "Cedro Rosa": 10, "Cedrela odorata": 10, # Engloba Cedro do Brejo
    ...
    "Pequi": 5, # Caryocar spp
    "Mangaba": 5, "Hancornia speciosa": 5,
    "Cagaita": 5, "Eugenia dysenterica Dc.": 5,
    "Guariroba": 5, "Syagrus oleracea": 5,
}

# --- CSS VISUAL (ALTO CONTRASTE E PREPARAÇÃO PARA IMPRESSÃO) ---
st.markdown("""
<style>
    /* Estilo de TELA (DARK MODE) */
    .dataframe { background-color: #1a1c24 !important; color: #ffffff !important; }
    ...
    /* Estilo de IMPRESSÃO (CTRL + P) */
    @media print {
        header, footer, .stButton, .stFileUploader, .stTabs [role="tablist"], .stAlert, button { display: none !important; }
        ...
        thead th { background-color: #f0f0f0 !important; color: black !important; font-weight: bold !important; }
        h1, h2, h3, h4, p, span, div, .stMetricValue, .stMetricLabel { color: black !important; }
    }
</style>
    ...
    if uploaded_file:
        try:
            ...
            if not all(c in df_raw.columns for c in colunas_obrigatorias):
                ...
                if dap_max > 200 or h_max > 60:
                    st.warning("⚠️ **Alerta:** Valores muito altos detectados. Verifique se não houve erro de digitação.")
        ...
        if st.button("🚀 Calcular Resultados", type="primary"):
            try:
                ...
                if n < 2:
                    ...
                    top_sp = df_calc['Nome Comum'].mode()[0] if 'Nome Comum' in df_calc.columns else "-"
                    top_fam = df_calc['Família'].mode()[0] if 'Família' in df_calc.columns else "-"

                    # --- NOVA FEATURE: CÁLCULO DE COMPENSAÇÃO (ART. 52) ---
                    
                    # 1. Agrupa o número de indivíduos amostrados por espécie e nome científico
                    df_contagem_amostra = df_calc.groupby(['Nome Comum', 'Nome Científico']).size().reset_index(name='N_Amostra')
                    
                    compensacao_list = []
                    
                    # Fator de Extrapolação Total (Área Total / Área Amostrada)
                    # Usamos a área total e o número de parcelas 'n' para o cálculo mais preciso
                    if Area_Amostrada_ha > 0:
                        Fator_Extrapolacao_pop = area_total_ha / Area_Amostrada_ha
                    else:
                        Fator_Extrapolacao_pop = 0 
                    ...
                    for index, row in df_contagem_amostra.iterrows():
                        ...
                        # 2. Verifica se está na lista protegida (prioriza Nome Científico)
                        if nome_cientifico in PROTECTED_SPECIES_MS:
                            compensacao_fator = PROTECTED_SPECIES_MS[nome_cientifico]
                        elif nome_comum in PROTECTED_SPECIES_MS:
                            compensacao_fator = PROTECTED_SPECIES_MS[nome_comum]
                            
                        if compensacao_fator > 0:
                            # 3. Extrapolação da População e Cálculo da Compensação
                            N_Estimado = n_amostra * Fator_Extrapolacao_pop
                            Mudas_Compensacao = np.ceil(N_Estimado) * compensacao_fator # Arredonda para cima antes de multiplicar
                    ...
                    st.session_state.resultados = {
                        "stats": {
                            ...
                            "ea": ea, "er": er, "ic_inf": ic_inf, "ic_sup": ic_sup,
                            "tot_vol": total_vol, "tot_ea": total_ea, 
                            "tot_ic_inf": total_ic_inf, "tot_ic_sup": total_ic_sup,
                            "n": n, "N": N, "area_total": area_total_ha,
                            "area_amostrada": Area_Amostrada_ha,
                    ...
                    # --- AUTO-NAVEGAÇÃO (Script para clicar na Tab 3) ---
                    components.html(
                        """
                        <script>
...
# ==============================================================================
# ABA 3: RELATÓRIO TÉCNICO (AGORA COM COMPENSAÇÃO)
# ==============================================================================
with tab3:
    if st.session_state.resultados is None:
        ...
        # --- NOVA SEÇÃO: COMPENSAÇÃO ART. 52 ---
        st.markdown("## 🌳 Compensação Obrigatória (Res. Semade n.9/2015)")
        
        if not df_comp.empty:
            total_compensacao = int(df_comp['Mudas_Compensacao'].sum())
            
            st.success(f"⚠️ **ATENÇÃO: ESPÉCIES PROTEGIDAS DETECTADAS!**")
            st.warning(f"O total de mudas para compensação exigido é de **{total_compensacao:,} mudas**.".replace(",", ".")) # Formatação de milhar
        ...
        st.download_button("📥 Baixar Tabela (CSV)", csv_t, "Resultados_Inventario.csv", "text/csv")
